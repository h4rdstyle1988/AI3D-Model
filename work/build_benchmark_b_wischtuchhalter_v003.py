#!/usr/bin/env python3
"""Build v003-r01 with the v002 geometry's vertical leg extended by 50 mm."""
...
CANDIDATE = "benchmark-b-wischtuchhalter-v003-r01"
...
PARAMETERS = {
    "candidate": CANDIDATE,
    "revision": "B-WISCHTUCH-2026-08-27.3",
    "material": "PETG",
...
def build_volume() -> tuple[np.ndarray, tuple[np.ndarray, np.ndarray, np.ndarray]]:
    ...
    solid |= rounded_box(x, y, z, (94.0, 0.0, 82.0), (8.0, 10.0, 16.0), TOUCH_RADIUS_MM)
    fillet_dx = x - 86.0
    fillet_dz = z - 86.0
    fillet_profile = (
        (x >= 86.0) & (x <= 90.0) & (z >= 82.0) & (z <= 86.0)
...
def render_installed(holder: trimesh.Trimesh, caddy_path: Path, output: Path) -> tuple[Path, Path]:
    ...
    scene.add_geometry(installed, node_name="wischtuchhalter-v003-r01")
