#!/usr/bin/env python3
"""Build the strictly specified single-hex-plug cloth rail revision v002-r01."""
...
CANDIDATE = "benchmark-b-wischtuchhalter-v002-r01"
...
PARAMETERS = {
    "candidate": CANDIDATE,
    "revision": "B-WISCHTUCH-2026-08-27.2",
    "material": "PETG",
...
def build_volume() -> tuple[np.ndarray, tuple[np.ndarray, np.ndarray, np.ndarray]]:
    ...
    # z=18 so the complete plug below remains an unmodified prism.
    bend_cx, bend_cz, centreline_radius = 10.0, PLUG_LENGTH_MM, 10.0
    dx = x - bend_cx
    dz = z - bend_cz
    ...
    bend = bend_cross & (theta >= math.pi / 2.0) & (theta <= math.pi) & (z >= PLUG_LENGTH_MM)
    solid |= bend

    # Main straight rail: bend ends tangentially at x=10, stop begins at x=90.
    rail_cross = rounded_rect_2d(y, z - 28.0, RAIL_WIDTH_MM / 2.0, RAIL_HEIGHT_MM / 2.0, TOUCH_RADIUS_MM)
    solid |= rail_cross & (x >= 10.0) & (x <= RAIL_PROJECTION_MM)

    # End stop: nominal 8-mm footprint and 8-mm rise.  The rounded box provides
    # R2 touch edges.  A conventional R4 internal fillet blends into the rail
    # over the preceding four millimetres without changing the 90-mm datum.
    solid |= rounded_box(x, y, z, (94.0, 0.0, 32.0), (8.0, 10.0, 16.0), TOUCH_RADIUS_MM)
    fillet_dx = x - 86.0
    fillet_dz = z - 36.0
    fillet_profile = (
        (x >= 86.0) & (x <= 90.0) & (z >= 32.0) & (z <= 36.0)
        & (fillet_dx * fillet_dx + fillet_dz * fillet_dz >= STOP_TRANSITION_RADIUS_MM ** 2)
    )
    # The exposed lateral edges of the small R4 filler are eased inside the
    # already rounded 10-mm rail envelope.
    lateral = np.abs(y) <= (RAIL_WIDTH_MM / 2.0 - TOUCH_RADIUS_MM / 2.0)
    solid |= fillet_profile & lateral
...
def render_installed(holder: trimesh.Trimesh, caddy_path: Path, output: Path) -> tuple[Path, Path]:
    ...
    scene.add_geometry(installed, node_name="wischtuchhalter-v002-r01")
