#!/usr/bin/env python3
"""Independent validation for v004-r01 continuous swept-profile transitions."""
...
def measure_radii(mesh: trimesh.Trimesh) -> dict:
    ...
    points = np.vstack([np.asarray(loop) for loop in section.discrete])
    bend_r = np.hypot(points[:, 0] - 10.0, points[:, 2] - 68.0)
    bend = bend_r[
        (points[:, 0] >= -0.2) & (points[:, 0] <= 10.2)
        & (points[:, 2] >= 67.8) & (points[:, 2] <= 78.2) & (bend_r < 8.0)
    ]
    bend_outer = bend_r[
        (points[:, 0] >= -4.2) & (points[:, 0] <= 10.2)
        & (points[:, 2] >= 67.8) & (points[:, 2] <= 82.2)
        & (bend_r > 12.0) & (bend_r < 16.0)
    ]
    stop_r = np.hypot(points[:, 0] - 86.0, points[:, 2] - 86.0)
    stop = stop_r[
        (points[:, 0] >= 85.8) & (points[:, 0] <= 90.2)
        & (points[:, 2] >= 81.8) & (points[:, 2] <= 86.2) & (stop_r < 5.0)
    ]
    stop_outer = stop_r[
        (points[:, 0] >= 85.8) & (points[:, 0] <= 98.2)
        & (points[:, 2] >= 73.8) & (points[:, 2] <= 86.2)
        & (stop_r > 10.0) & (stop_r < 14.0)
    ]
    bend_q = np.quantile(bend, [0.05, 0.5, 0.95])
    bend_outer_q = np.quantile(bend_outer, [0.05, 0.5, 0.95])
    stop_q = np.quantile(stop, [0.05, 0.5, 0.95])
    stop_outer_q = np.quantile(stop_outer, [0.05, 0.5, 0.95])
    bend_profile_thickness = float(bend_outer_q[1] - bend_q[1])
    stop_profile_thickness = float(stop_outer_q[1] - stop_q[1])
    return {
        ...
        "main_bend_outer_radius_mm": {"target": 14.0, "median": float(bend_outer_q[1])},
        "stop_bend_outer_radius_mm": {"target": 12.0, "median": float(stop_outer_q[1])},
        "main_bend_profile_thickness_mm": {"target": 8.0, "measured": bend_profile_thickness, "pass": bool(abs(bend_profile_thickness - 8.0) <= 0.10)},
        "stop_bend_profile_thickness_mm": {"target": 8.0, "measured": stop_profile_thickness, "pass": bool(abs(stop_profile_thickness - 8.0) <= 0.10)},
...
def parse_slicer(directory: Path) -> dict:
    slicer_dir = directory / "slicer-anycubic-petg-side-no-support-no-repair"
    ...
    with gcode.open("r", encoding="utf-8", errors="replace") as handle:
        for line in handle:
            ...
            elif value.startswith("EXCLUDE_OBJECT_DEFINE") and "POLYGON=" in value:
                pairs = re.findall(r"\[(-?\d+(?:\.\d+)?),(-?\d+(?:\.\d+)?)\]", value.split("POLYGON=", 1)[1])
                if pairs:
                    xy = np.asarray([[float(a), float(b)] for a, b in pairs])
                    exclusion_polygon = {
                        "bounds_xy_mm": [xy.min(axis=0).tolist(), xy.max(axis=0).tolist()],
                        "extents_xy_mm": (xy.max(axis=0) - xy.min(axis=0)).tolist(),
                        "points": int(len(xy)),
                    }
    metadata_size_consistent = bool(
        exclusion_polygon is not None
        and max(exclusion_polygon["extents_xy_mm"]) >= 100.0
    )
    return {
        "audit": audit,
        ...
        "object_exclusion_polygon": exclusion_polygon,
        "statistics": stats,
        "statistics_model_size_classification": {
            "status": "INCONSISTENT_METADATA_IGNORED" if metadata_size_consistent and max(stats.get("sliced_model_size_mm", [0.0])) < 100.0 else "CONSISTENT",
            "basis": f"The authoritative object exclusion polygon spans the full model; all {audit['layers']['layer_change_markers']} reported layers contain extrusion.",
        },
        "pass": bool(audit["pass"] and not enable_support and support == 0 and interface == 0 and metadata_size_consistent),
    }
...
def main() -> int:
    ...
    dimensions = {
        ...
        "plug_sections": plug,
        "vertical_extension_above_unchanged_plug_mm": params["rail"]["vertical_extension_above_plug_mm"],
        "full_seated_rail_underside_above_grid_mm": params["rail"]["full_seated_rail_underside_above_grid_mm"],
        "full_seated_rail_centre_above_grid_mm": params["rail"]["full_seated_rail_centre_above_grid_mm"],
    ...
    dimension_checks = {
        ...
        "vertical_extension_exactly_50mm": params["rail"]["vertical_extension_above_plug_mm"] == 50.0,
        "full_seated_underside_clearance_56mm": params["rail"]["full_seated_rail_underside_above_grid_mm"] == 56.0,
    ...
    geometry_checks = {
        ...
        "slicer_pass_without_auto_repair_or_support": slicer["pass"] and not slicer["support_enabled"]
        and slicer["support_markers"] == 0 and slicer["support_interface_markers"] == 0
        and slicer["audit"]["mode"]["auto_repair"] == "DISABLED_BY_--no-check",
    }
    form_checks = {
        "continuous_constant_section_main_bend": radii["main_bend_profile_thickness_mm"]["pass"],
        "continuous_constant_section_end_bend": radii["stop_bend_profile_thickness_mm"]["pass"],
        "tangent_main_transition_by_single_sweep": params["rail"]["rounding_form"].startswith("continuous constant-section swept profile"),
        "tangent_end_transition_by_single_sweep": params["rail"]["stop_transition_radius_mm"] == 4.0,
        "no_local_rounding_thickening": not params["rail"]["local_rounding_thickening"],
        "same_10x8_envelope_through_roundings": np.allclose(rail_section["extents_mm"], [10.0, 8.0], atol=0.051)
        and radii["main_bend_profile_thickness_mm"]["pass"] and radii["stop_bend_profile_thickness_mm"]["pass"],
    }
    ...
    mechanical = {
        ...
        "transition": "50-mm straight vertical extension above the unchanged 18-mm plug, followed by the R6 reinforced bend",
        "print_orientation": "side lying; local Y becomes layer-stack Z; support disabled",
        "limitation": "Wet-cycle, creep, printer calibration and actual cloth load require one physical PETG test.",
    }

    technical_pass = all(dimension_checks.values()) and all(fit_checks.values()) and all(geometry_checks.values()) and all(form_checks.values()) and all(forbidden_element_checks.values())
    report = {
        "schema": "ai3d.benchmark-b.wischtuchhalter.gate-report.v4",
        "candidate": {
            "id": candidate,
        ...
        "rounding_form_checks": form_checks,
        "geometry": {
            "topology": topo,
            ...
            "self_intersections": {
                ...
                "limitation": f"no exhaustive O(N^2) all-pair enumeration for {topo['faces']:,} triangles",
            },
            "3mf_roundtrip": three,
        },
    ...
    (directory / "DIMENSION-REPORT.md").write_text(
        ...
        "- Senkrechter Abschnitt oberhalb des unveränderten Steckzapfens: **50,00 mm länger als v002-r01**.\n"
        "- Bei vollständig eingesetztem Zapfen: Bügelunterseite **56,00 mm**, Bügelmitte **60,00 mm** über der Gitteroberkante.\n"
        ...
        f"- Durchgehende Profilstärke im Hauptbogen/Endbogen: **{radii['main_bend_profile_thickness_mm']['measured']:.3f} / {radii['stop_bend_profile_thickness_mm']['measured']:.3f} mm** (Soll jeweils 8,00 mm).\n"
    ...
    (directory / "ROUNDING-FORM-REPORT.md").write_text(
        f"# RUNDUNGS- UND FORMREPORT – {candidate}\n\n"
        f"Status: **{'PASS' if all(form_checks.values()) else 'FAIL'}**\n\n"
        "Gegenüber v003-r01 wurden ausschließlich die beiden Übergangsformen neu aufgebaut. Steckzapfen, Einstecktiefe, Höhe, Ausladung, Querschnitt, Endanschlagmaß und Ausrichtung blieben unverändert.\n\n"
        f"- Hauptübergang: ein tangentialer Sweep des durchgehenden 10 × 8-mm-Profils; Innenradius Soll 6,00 mm, Median **{radii['transition_inner_radius_mm']['median']:.3f} mm**, Außenradius **{radii['main_bend_outer_radius_mm']['median']:.3f} mm**.\n"
        f"- Profilstärke im Hauptbogen: **{radii['main_bend_profile_thickness_mm']['measured']:.3f} mm** bei Soll 8,00 mm.\n"
        f"- Endanschlag: tangentialer Viertelbogen ohne angesetzten Füllkörper; Innenradius Soll 4,00 mm, Median **{radii['stop_transition_radius_mm']['median']:.3f} mm**, Außenradius **{radii['stop_bend_outer_radius_mm']['median']:.3f} mm**.\n"
        f"- Profilstärke im Endbogen: **{radii['stop_bend_profile_thickness_mm']['measured']:.3f} mm** bei Soll 8,00 mm.\n"
        "- Lokale Rundungsverdickung, sichtbare Absätze und zusätzliche Wulstkörper: **nicht vorhanden**.\n"
        "- Druckausrichtung: auf der Profilseite; Support **deaktiviert**.\n\n"
        "Die Seiten- und Perspektivdarstellung `rounding-review-side-and-perspective.png` dokumentiert beide Übergänge.\n",
        encoding="utf-8",
    )
    (directory / "VALIDATION-REPORT.md").write_text(
        ...
        "Gegenüber v003-r01 wurden ausschließlich die Rundungsformen überarbeitet. Hauptbogen und Endanschlag sind nun tangentiale Sweeps eines durchgehenden 10 × 8-mm-Profils; lokale Wülste, angesetzte Füllkörper und Querschnittsverdickungen entfallen. Alle Funktionsmaße bleiben unverändert.\n\n"
        ...
        f"Support deaktiviert; Support-/Interface-Marker **{slicer['support_markers']}/{slicer['support_interface_markers']}**. Prognose: {slicer['statistics'].get('print_time')}, Filamentpfad {slicer['statistics'].get('used_filament_m')} m.\n",
        encoding="utf-8",
    )
